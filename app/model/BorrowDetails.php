<?php 
class BorrowDetails extends DB{
	function __construct(DB $conn){
		$this->conn = $conn->conn; 
	}


	protected static $tbl_name = "borrow_details";
	// Select All
	public function all(){  
	...
	public function get_by_borrow_id($id){  
		$sql = "SELECT * FROM ".self::$tbl_name." WHERE borrow_id = :id";
		$stmt = $this->conn->prepare($sql);
		$stmt->bindValue(':id', $id);
		$stmt->execute();
		return $stmt->fetchAll(PDO::FETCH_OBJ);
	...
	public function delete_by_borrow_id($id=0) {
